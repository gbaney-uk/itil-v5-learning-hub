/* ITIL V5 Learning Dashboard — Phase 3 app engine */
(function () {
  "use strict";

  const APP_VERSION = "3.0";
  const STORAGE_PREFIX = "itil_v5_learning_phase3";
  const PASS_MARK = 65;
  ...
  function renderMindMapTab(section) {
    ...
    if (!maps.length) {
      return `
        <div class="mindmap-empty">
          ...
          <span>Add an HTML mind map for this section under <code>assets/mindmaps/</code>, then reference it in <code>content.js</code>.</span>
        </div>
      `;
    }
    ...
    return `
      <div class="mindmap-intro">
        ...
        <span>These HTML mind maps sit inside the app and can also be used as the first validation layer for section questions.</span>
      </div>
      ${maps.map((map, index) => `
        <article class="mindmap-card">
          <div class="mindmap-card-head">
            <div>
              ...
              <span>${escapeHtml(map.sourceNote || "Mind map source for this topic.")}</span>
            </div>
            <a class="mini-link" href="${escapeHtml(map.file)}" target="_blank" rel="noopener">Open full page</a>
          </div>
          <iframe
            class="mindmap-frame"
            title="${escapeHtml(map.title)} mind map"
            src="${escapeHtml(map.file)}"
            loading="lazy"></iframe>
        </article>
      `).join("")}
    `;
